#!/bin/sh

set -e

auto_confirm=0
purge_env=0

confirm () {
    if [ "$auto_confirm" = "1" ]; then
        return 0
    fi

    printf "Are you sure you want to proceed? (y/N) "
    read -r REPLY
    case "$REPLY" in
        [Yy])
            ;;
        *)
            echo "Script canceled."
            exit 1
            ;;
    esac
}

for arg in "$@"; do
    case "$arg" in
        -y|--yes)
            auto_confirm=1
            ;;
        --purge-env|--purge)
            purge_env=1
            ;;
    esac
done

echo ""
echo "*** WARNING: This will remove all containers and container data, and optionally reset .env ***"
echo ""

confirm

echo "===> Stopping and removing known project containers..."
if command -v docker >/dev/null 2>&1; then
    ids="$(docker ps -aq \
        --filter name=portainer \
        --filter name=npm \
        --filter name=daiana \
        --filter name=supabase \
        --filter name=studio \
        --filter name=kong \
        --filter name=auth \
        --filter name=postgrest \
        --filter name=realtime \
        --filter name=storage \
        --filter name=meta \
        --filter name=db \
        --filter name=mail \
        || true)"
    if [ -n "$ids" ]; then
        # shellcheck disable=SC2086
        docker rm -f $ids >/dev/null 2>&1 || true
    fi
fi

echo "===> Stopping and removing all containers..."

if [ -f ".env" ]; then
    docker compose -f docker-compose.yml -f ./dev/docker-compose.dev.yml down -v --remove-orphans
elif [ -f ".env.example" ]; then
    echo "No .env found, using .env.example for docker compose down..."
    docker compose --env-file .env.example -f docker-compose.yml -f ./dev/docker-compose.dev.yml down -v --remove-orphans
else
    echo "Skipping 'docker compose down' because there's no env-file."
fi

echo "===> Cleaning up bind-mounted directories..."
BIND_MOUNTS="./volumes/db/data ./volumes/storage"

for dir in $BIND_MOUNTS; do
    if [ -d "$dir" ]; then
        echo "Removing $dir..."
        confirm
        rm -rf "$dir"
    else
        echo "$dir not found."
    fi
done

echo "===> Resetting .env file (will keep .env.old if it already exists)..."
confirm
if [ "$purge_env" = "1" ]; then
    echo "Purge mode enabled: removing .env and .env.old"
    rm -f .env .env.old
elif [ -f ".env" ] || [ -L ".env" ]; then
    if [ -f ".env.old" ] || [ -L ".env.old" ]; then
        echo "Keeping existing .env.old and removing .env"
        rm -f .env
    else
        echo "Renaming existing .env file to .env.old"
        mv .env .env.old
    fi
else
    echo "No .env file found."
fi

if [ "$purge_env" != "1" ]; then
    if [ -f ".env.example" ]; then
        echo "===> Copying .env.example to .env"
        cp .env.example .env
    else
        echo "No .env.example found, can't restore .env to default values."
    fi
else
    echo "Purge mode: not recreating .env"
fi

echo "Cleanup complete!"
echo "Re-run 'docker compose pull' to update images."
echo ""
